function CSTR_Tausif()
    ...
    % Reaction Rate Constant k0 (log scale)
    uicontrol('Style', 'text', 'Units', 'normalized', ...
    ...
    function editCallback(param)
        ...
        % Validate input & Restore Previous Value if NaN
        if isnan(val)
            slider = findobj('Tag', [param '_slider']);
    ...
    function updatePlot()
        ...
        try
            ...
            plot(ax, [0 10], [p.CA0 p.CA0], 'k--', 'LineWidth', 1, 'DisplayName', 'CA0');
            hold(ax, 'off');
